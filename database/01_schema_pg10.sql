...
-- ---------------------------------------------------------------------------
-- Clean up any partial run from a previous failed attempt.
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.gen_uuid_v4() CASCADE;
DROP TYPE IF EXISTS package_type CASCADE;
DROP TYPE IF EXISTS representative_status CASCADE;
...
DROP TYPE IF EXISTS disciplinary_action_type CASCADE;
...
DROP TYPE IF EXISTS beneficiary_role CASCADE;
DROP TYPE IF EXISTS distribution_type CASCADE;
DROP TYPE IF EXISTS project_status CASCADE;
DROP TYPE IF EXISTS payout_schedule CASCADE;

-- ---------------------------------------------------------------------------
-- Pure PL/pgSQL UUID v4 generator (no extensions required).
-- Compatible with PostgreSQL 10+ using only built-in plpgsql.
-- Uses $body$ quoting to avoid any shell escaping issues.
-- ---------------------------------------------------------------------------
...
DECLARE
    v TEXT;
BEGIN
    v := lpad(to_hex(floor(random()*4294967296)::bigint), 8, '0') || '-' ||
         lpad(to_hex(floor(random()*65536)::bigint), 4, '0') || '-' ||
         '4' || lpad(to_hex(floor(random()*4096)::bigint), 3, '0') || '-' ||
...
-- USERS â€” auth handled at app layer (password_hash), no Supabase auth.users FK
CREATE TABLE users (
    id                  UUID PRIMARY KEY DEFAULT public.gen_uuid_v4(),
...
-- PACKAGES (Standard / Premium) â€” all amounts configurable
